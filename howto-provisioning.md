...
## Provisioning through the the {{site.data.keyword.cloud_notm}} console
{: #catalog}
{: ui}
...
1. Provision a {{site.data.keyword.databases-for-postgresql}} Shared instance with a command like:
   ...
   Provision a {{site.data.keyword.databases-for-postgresql}} Isolated instance with the same `"members_host_flavor"` -p flag, and then specify the `host_flavor value` parameter. For example: `{"members_host_flavor": "b3c.4x16.encrypted"}`
